...
(() => {
  ...
  const state = {
    ...
    localStream: null, cameraTrack: null, screenTrack: null, peers: new Map(), participants: new Map(), pendingIce: new Map(), pendingSignals: new Map(),
    ...
    remoteAudio: new Map(), audioGraph: new Map(), unread: 0, speakerTestTimer: null, facingMode: 'user'
  };
  ...
  async function getMedia() {
    ...
    els.preview.autoplay = true;
    els.preview.playsInline = true;
    els.preview.muted = true;
    els.preview.volume = 0;
    els.preview.srcObject = stream;
    els.previewFallback.classList.toggle('hidden', !!state.cameraTrack);
    // Start the lobby preview reliably on browsers that delay playback after srcObject changes.
    requestAnimationFrame(() => els.preview.play().catch(() => {}));
    startMicMeter(mic);
    updateControls();
    return stream;
  }
  ...
  function joinOrCreate() {
    ...
    else if (state.roomId) state.socket.emit('join-room', { roomId: state.roomId, name: state.name }, async res => {
      if(!res?.ok){ showError(res?.error || 'Could not join room.', true); return; }
      state.selfId=res.selfId; state.role='student'; state.joined=true;
      applyRoom(res.room);
  ...
  function recoverAfterReconnect() {
    ...
    closeAllPeers(); state.joined=false; state.participants.clear();
    if(state.roomId) state.socket.emit('join-room',{roomId:state.roomId,name:state.name},async res=>{
      if(!res?.ok){showError(res?.error||'Could not reconnect.',true);return;}
      state.selfId=res.selfId;
      applyRoom(res.room);
      state.joined=true;
      for(const p of res.existing||[]){state.participants.set(p.id,p);ensurePeer(p.id,false)}
  ...
  async function enterMeeting(alreadyJoined=false) {
    if (!state.socket?.connected) return showError('Connecting to meeting server…', true);
    state.joined=true; state.intentionalLeave=false;
    // Attach the already-tested local camera stream to the meeting tile immediately.
    // This is intentionally done after the Enter click so browser autoplay policies are satisfied.
    els.overlay.classList.add('hidden');
    renderGrid();
    const localCard = state.selfId ? qs(`[data-id="${CSS.escape(state.selfId)}"]`) : null;
    if(localCard) attachLocalVideo(localCard.querySelector('video'));
    renderPeople(); updateControls(); startStats();
  ...
  function queueSignal(fromId, type, data){
    const q=state.pendingSignals.get(fromId)||{offers:[],answers:[],ice:[]};
    if(type==='offer') q.offers.push(data);
    else if(type==='answer') q.answers.push(data);
    else q.ice.push(data);
    state.pendingSignals.set(fromId,q);
  }
  async function flushPendingSignals(){
    if(!state.joined) return;
    const entries=[...state.pendingSignals.entries()];
    state.pendingSignals.clear();
    for(const [id,q] of entries){
      for(const data of q.offers) await handleOffer(data);
      for(const data of q.answers) await handleAnswer(data);
      for(const data of q.ice) await handleIce(data);
    }
  }
  async function makeOffer(id,pc,iceRestart=false) {
    try {
      if(pc.signalingState==='closed') return;
      const offer=await pc.createOffer(iceRestart?{iceRestart:true}:undefined);
      await pc.setLocalDescription(offer);
      state.socket.emit('offer',{targetId:id,description:pc.localDescription});
    } catch(e){ console.warn('offer',e); }
  }
  async function handleOffer({fromId,description}) {
    if(!state.joined || !state.localStream){ queueSignal(fromId,'offer',{fromId,description}); return; }
    const pc=ensurePeer(fromId,false); if(!pc)return;
    try {
      // Ignore stale offers after a peer has already been closed/replaced.
      if(pc.signalingState==='closed') return;
      await pc.setRemoteDescription(description);
      await flushIce(fromId);
      const answer=await pc.createAnswer();
      await pc.setLocalDescription(answer);
      state.socket.emit('answer',{targetId:fromId,description:pc.localDescription});
    } catch(e){ console.warn('answer',e); }
  }
  async function handleAnswer({fromId,description}) {
    if(!state.joined){ queueSignal(fromId,'answer',{fromId,description}); return; }
    const pc=state.peers.get(fromId);
    if(!pc || pc.signalingState==='closed') { queueSignal(fromId,'answer',{fromId,description}); return; }
    try{
      // An answer is valid only while the peer is waiting for one.
      if(pc.signalingState!=='have-local-offer') return;
      await pc.setRemoteDescription(description);
      await flushIce(fromId);
    }catch(e){console.warn('remote answer',e);}
  }
  async function handleIce({fromId,candidate}) {
    if(!state.joined || !state.localStream){ queueSignal(fromId,'ice',{fromId,candidate}); return; }
    const pc=state.peers.get(fromId)||ensurePeer(fromId,false); if(!pc)return;
    if(pc.remoteDescription?.type){try{await pc.addIceCandidate(candidate)}catch(e){console.warn('ICE',e)}}
    else{const q=state.pendingIce.get(fromId)||[];q.push(candidate);state.pendingIce.set(fromId,q);}
  }
  async function flushIce(id){const pc=state.peers.get(id);if(!pc)return;const q=state.pendingIce.get(id)||[];for(const c of q){try{await pc.addIceCandidate(c)}catch{}}state.pendingIce.set(id,[]);}
  ...
  function setupRemoteAudio(video, id, stream){
    if(!stream) return;
    let audio=state.remoteAudio.get(id);
    if(!audio){
      ...
      state.remoteAudio.set(id,audio);
    }
    if(audio.srcObject!==stream) audio.srcObject=stream;
    const AudioCtx=window.AudioContext||window.webkitAudioContext;
    if(AudioCtx && !state.audioGraph.has(id)){
      try{
        ...
        state.audioGraph.set(id,{ctx,source,compressor,gain,audio});
        audio.addEventListener('play',()=>ctx.resume().catch(()=>{}));
        ctx.resume().catch(()=>{});
      }catch(e){ console.warn('remote audio graph',e); }
    }else{
      state.audioGraph.get(id)?.ctx?.resume?.().catch?.(()=>{});
    }
    audio.play().then(()=>state.audioGraph.get(id)?.ctx?.resume?.()).catch(()=>{});
    // The visual video must not also play the same audio, otherwise it would double the sound.
    video.muted=true;
    video.volume=0;
  }
  function closeRemoteAudio(id){
    const g=state.audioGraph.get(id); if(g){try{g.source.disconnect();g.compressor.disconnect();g.gain.disconnect();}catch{} try{g.ctx.close();}catch{} state.audioGraph.delete(id);}
    const audio=state.remoteAudio.get(id); if(audio){try{audio.pause();audio.srcObject=null;audio.remove();}catch{} state.remoteAudio.delete(id);}
  }
  function attachRemoteTrack(id,stream,track){
    if(!stream) return;
    const card=ensureCard(id); const video=card.querySelector('video');
    video.autoplay=true; video.playsInline=true; video.muted=true; video.volume=0;
    if(video.srcObject!==stream) video.srcObject=stream;
    if(track.kind==='audio') track.enabled=true;
    setupRemoteAudio(video,id,stream);
    video.play().catch(()=>{});
    updatePiPButton(video);
  }
  ...
  function attachLocalVideo(video){
    ...
    const playLocal = () => video.play().catch(() => {
      setTimeout(() => video.play().catch(() => {}), 100);
    });
    if(video.readyState >= 2) playLocal();
    else video.addEventListener('loadedmetadata', playLocal, {once:true});
    requestAnimationFrame(playLocal);
  }

  function renderGrid(){
    const all=[...state.participants.values()];
    els.empty.classList.toggle('hidden',all.length>1);
    const wanted=new Set(all.map(p=>p.id));
    [...els.grid.querySelectorAll('.video-card')].forEach(card=>{
      if(!wanted.has(card.dataset.id)) card.remove();
    });
    for(const p of all){
      const card=ensureCard(p.id);
      const video=card.querySelector('video');
      const avatar=card.querySelector('.avatar');
      const name=card.querySelector('.name-pill');
      const status=card.querySelector('.state-pill');
      name.textContent=displayName(p);
      status.textContent=`${p.role==='teacher'?'Teacher':'Student'} • ${p.micOn?'🎙':'🔇'} ${p.cameraOn?'▣':'□'}${p.handRaised?' • ✋':''}`;
      const cameraVisible=p.id===state.selfId ? state.cameraOn : p.cameraOn;
      card.classList.toggle('audio-only',state.audioOnly||!cameraVisible);
      avatar.textContent=initials(p.name);
      if(p.id===state.selfId){
        if(state.localStream && state.cameraOn && !state.audioOnly){
          card.classList.remove('audio-only');
          attachLocalVideo(video);
        }
      }else if(video.srcObject){
        video.muted=true;
        video.volume=0;
        video.play().catch(()=>{});
        setupRemoteAudio(video,p.id,video.srcObject);
      }
      updatePiPButton(video);
    }
    const localCard=state.selfId ? qs(`[data-id="${CSS.escape(state.selfId)}"]`) : null;
    if(localCard && state.localStream) attachLocalVideo(localCard.querySelector('video'));
  }
  ...
  function removeParticipant(id){state.participants.delete(id);const pc=state.peers.get(id);pc?.close();state.peers.delete(id);state.pendingIce.delete(id);closeRemoteAudio(id);qs(`[data-id="${CSS.escape(id)}"]`)?.remove();renderPeople();renderGrid();}
  function closeAllPeers(){for(const pc of state.peers.values())pc.close();state.peers.clear();state.pendingIce.clear();for(const id of [...state.remoteAudio.keys()])closeRemoteAudio(id);state.pendingSignals.clear();}
  ...
  async function testSpeaker(){if(state.speakerTestTimer)return;const C=window.AudioContext||window.webkitAudioContext;if(!C)return showError('Speaker test is not supported by this browser.');try{const ctx=new C();await ctx.resume();const osc=ctx.createOscillator();const gain=ctx.createGain();const comp=ctx.createDynamicsCompressor();osc.type='sine';osc.frequency.value=660;gain.gain.setValueAtTime(0.0001,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(.035,ctx.currentTime+.04);gain.gain.exponentialRampToValueAtTime(.0001,ctx.currentTime+.55);comp.threshold.value=-24;comp.knee.value=18;comp.ratio.value=3;comp.attack.value=.004;comp.release.value=.2;osc.connect(gain).connect(comp).connect(ctx.destination);osc.start();state.speakerTestTimer=setTimeout(()=>{try{osc.stop();ctx.close()}catch{}state.speakerTestTimer=null},650)}catch{showError('Speaker test failed.');}}
  ...
  function syncMobilePanel(){
    ...
    if(window.matchMedia('(max-width:900px)').matches) els.sidePanel.classList.add('hidden-mobile');
    else els.sidePanel.classList.remove('hidden-mobile');
  }
