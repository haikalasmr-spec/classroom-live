...
(() => {
  ...
  function joinOrCreate() {
    ...
    else if (state.roomId) state.socket.emit('join-room', { roomId: state.roomId, name: state.name }, res => {
  ...
  function setupRemoteAudio(video, id){
    if(!video || video.muted) return;
    if(state.audioGraph.has(id) && state.audioGraph.get(id).video === video) return;
    try{
      const AudioCtx=window.AudioContext||window.webkitAudioContext;
      if(!AudioCtx) return;
      const ctx=state.audioGraph.get(id)?.ctx || new AudioCtx();
      ...
      state.audioGraph.set(id,{ctx,source,compressor,gain,video});
  ...
  function closeRemoteAudio(id){
    const g=state.audioGraph.get(id); if(!g)return;
    try{g.source.disconnect();g.compressor.disconnect();g.gain.disconnect();}catch{}
    try{g.ctx.close();}catch{}
    state.audioGraph.delete(id);
  }
  function attachRemoteTrack(id,stream,track){
    const card=ensureCard(id); const video=card.querySelector('video');
    video.autoplay=true; video.playsInline=true; video.muted=false; video.volume=1;
    if(video.srcObject!==stream) video.srcObject=stream;
    if(track.kind==='audio') track.enabled=true;
    setupRemoteAudio(video,id);
    video.play().then(()=>state.audioGraph.get(id)?.ctx.resume().catch(()=>{})).catch(()=>{});
  ...
  function renderGrid(){
    ...
    for(const p of all){
      ...
      if(p.id===state.selfId){
        ...
        video.play().then(()=>state.audioGraph.get(p.id)?.ctx.resume().catch(()=>{})).catch(()=>{});
        setupRemoteAudio(video,p.id);
      }
      updatePiPButton(video);
    }
    const localCard=state.selfId ? qs(`[data-id="${CSS.escape(state.selfId)}"]`) : null;
  ...
  function removeParticipant(id){state.participants.delete(id);const pc=state.peers.get(id);pc?.close();state.peers.delete(id);state.pendingIce.delete(id);state.remoteAudio.get(id)?.pause();state.remoteAudio.delete(id);closeRemoteAudio(id);qs(`[data-id="${CSS.escape(id)}"]`)?.remove();renderPeople();renderGrid();}
  function closeAllPeers(){for(const pc of state.peers.values())pc.close();state.peers.clear();state.pendingIce.clear();for(const v of state.remoteAudio.values())v.pause();state.remoteAudio.clear();for(const id of [...state.audioGraph.keys()])closeRemoteAudio(id);}
